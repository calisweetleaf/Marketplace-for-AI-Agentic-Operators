...
Use `/plan` to enter plan mode explicitly. You can also provide an initial planning prompt with `/plan <description>`.

Use `/view-plan` to open the current saved plan preview after a plan exists. `/view-plan` aliases: `/show-plan`, `/plan-view`.
