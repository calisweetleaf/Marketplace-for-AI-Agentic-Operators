...
The first file found in each directory is used. `Claude.md`, `CLAUDE.md`, and `CLAUDE.local.md` are supported for compatibility with Claude Code workflows. Additionally, `~/.claude/CLAUDE.md` and `~/.claude/CLAUDE.local.md` are scanned at the home level when Claude compatibility is enabled (the default).

### Rules Directories

In addition to AGENTS.md files, Grok scans for `*.md` files in rules directories within the current working directory's path:

| Location | Notes |
|----------|-------|
| `<cwd>/.grok/rules/` | Always scanned |
| `<cwd>/.claude/rules/` | Claude compatibility (configurable) |
| `<cwd>/.cursor/rules/` | Cursor compatibility (configurable) |

The Claude and Cursor rules directories are scanned by default. To disable scanning for a specific vendor, use the `[compat]` config section or the corresponding environment variable. See [Configuration](05-configuration.md#vendor-compatibility) for details.
