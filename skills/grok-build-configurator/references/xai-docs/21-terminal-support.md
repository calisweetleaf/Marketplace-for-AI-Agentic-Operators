...
**Cause**: WezTerm ships with Kitty keyboard protocol disabled. Grok relies on it to tell `Ctrl+Enter` (interject) and `Shift+Enter`/`Ctrl+Enter` (in multiline mode) apart from plain `Enter`. Most other terminals turn it on by default.
...
**Verify**: Run `/terminal-setup` inside Grok. When a turn is active you should see the interject hint and `Ctrl+Enter` should work.
