...
Control whether Grok scans Cursor (`.cursor/`) and Claude (`.claude/`) config directories for skills, rules, project instructions, MCP servers, and hooks. All cells default to `true` (on), preserving existing behavior.
