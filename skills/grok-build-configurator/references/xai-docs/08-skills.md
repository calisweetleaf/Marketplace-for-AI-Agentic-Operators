...
| `~/.claude/skills/` | User | Lowest | Claude Code compatibility (configurable) |
...
The Claude and Cursor skill directories are scanned by default for compatibility. To disable scanning for a specific vendor, set the corresponding `[compat]` option in `~/.grok/config.toml` or environment variable. See [Configuration](05-configuration.md#vendor-compatibility) for details. Known vendor-shipped default skills (e.g. Cursor's built-in `shell`, `canvas`, `statusline`) are automatically filtered out regardless of the toggle.
