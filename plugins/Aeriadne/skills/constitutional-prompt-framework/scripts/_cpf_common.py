...
def load_json_compatible_yaml(path: Path):
    """Load a YAML file that is also valid JSON (i.e., JSON-superset YAML only).

    All current CPF eval files are authored as JSON arrays for stdlib-only
    compatibility. If the file is ever authored in full YAML syntax this will
    raise a clear ValueError rather than a confusing JSONDecodeError.
    """
    text = path.read_text(encoding="utf-8")
    try:
        ...
        raise ValueError(
            f"Eval file {path} is not valid JSON-compatible YAML.\n"
            f"CPF eval files must be authored as JSON arrays (stdlib-only constraint).\n"
            f"Parse error: {exc}"
        ) from exc
