...
## Section 7: Monorepo Protocol

When operating in a monorepo where multiple subdirectories may be independent CTMv3 activations, standard boot discovery on the root may yield `COLD_START` even if subdirectories are activated. 

To discover all activations across a monorepo, use the `discover_all()` API directly:
...
for inv in inventories:
    print(f"{inv.project_root}: {inv.branch}")
```

**Monorepo Rules:**
1. Do NOT run `activate` on the monorepo root expecting it to cascade to subdirectories. Activation is always scoped to the `project_root` passed.
2. If the root contains global configuration (e.g., a root `pyproject.toml` workspace definition), it can be activated independently of its subdirectories.
3. Agents should navigate to the specific subdirectory they intend to work on, and run standard boot discovery from there, treating that subdirectory as the active `project_root`.
