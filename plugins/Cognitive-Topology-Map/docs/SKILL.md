...
description: >
  Cognitive Topology Map (CTM) v3 skill creation engine for building or upgrading
  Codex skills, AGENTS.md or CLAUDE.md files, ARCHITECTURE_MAP.md traversal maps,
  and agent ecosystem artifacts such as .github, .claude, .codex, and .sovereign.
  Use when Daeron asks to make a skill, encode a codebase, build Codex context,
  document a repo for autonomous agents, cold-start or warm-start a repo, set up
  agent hooks, or integrate agents into a project. Produces navigable topology,
  failure grammar, provenance, boot-state, and executable ecosystem artifacts,
  not generic workflow checklists.
